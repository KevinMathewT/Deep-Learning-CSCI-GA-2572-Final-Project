    ...
    def forward(self, states, actions, teacher_forcing=True):
        ...
        if teacher_forcing:
            ...
            for t in range(1, T):
                ...
                pred_state = self.pred(state_embed_t_minus1, action_t_minus1)  # (B, embed_dim)
...
class AdversarialJEPA(BaseModel):
    def __init__(self, config):
        ...
        self.scheduler = OneCycleLR(self.opt, max_lr=config.learning_rate, steps_per_epoch=config.steps_per_epoch, epochs=config.epochs)
        self.scheduler_disc = OneCycleLR(self.disc_opt, max_lr=config.learning_rate, steps_per_epoch=config.steps_per_epoch, epochs=config.epochs)
    ...
    def compute_losses(self, preds, enc_s):
        ...
        gen_preds = self.disc(preds[:, 1:].detach().reshape(-1, self.config.embed_dim))  # (B*(T-1), 1)
    ...
    def training_step(self, batch, device):
        ...
        learning_rate = self.opt.param_groups[0]['lr']  # Assuming same LR for all optimizers
        disc_learning_rate = self.disc_opt.param_groups[0]['lr']  # Assuming same LR for all optimizers
...
class ActionRegularizationJEPA(BaseModel):
    ...
    def compute_regularization_loss(self, states_embed, pred_states, actions):
        ...
        embedding_diff = pred_states - states_embed  # Difference between input and output of predictor
        actions = actions.view(-1, self.config.action_dim)  # Flatten actions to (B*(T-1), action_dim)

        # Predict actions from embedding differences
        predicted_actions = self.reg_net(embedding_diff)  # (B*(T-1), action_dim)
    ...
    def training_step(self, batch, device):
        ...
        states_embed = enc_s[:, :-1, :].reshape(-1, self.config.embed_dim)  # Input to predictor
        pred_states = preds[:, 1:, :].reshape(-1, self.config.embed_dim)  # Output of predictor
        action_reg_loss = self.compute_regularization_loss(states_embed, pred_states, actions)


        vic_reg_loss, invariance_loss, variance_loss, covariance_loss = self.compute_vicreg_loss(preds, enc_s) # VICReg Loss Calculation
